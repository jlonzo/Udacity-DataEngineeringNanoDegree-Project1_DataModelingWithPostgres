...
songplay_table_create = ("CREATE TABLE IF NOT EXISTS songplays ( \
    songplay_id SERIAL PRIMARY KEY, start_time timestamp, user_id text, \
    level text, song_id text, artist_id text, session_id int, location text, \
    user_agent text)")

user_table_create = ("CREATE TABLE IF NOT EXISTS users( \
    user_id int PRIMARY KEY, first_name text, last_name text, gender char(1), \
    level text )")

song_table_create = ("CREATE TABLE IF NOT EXISTS songs( \
    song_id text PRIMARY KEY, title text, artist_id text, year smallint, \
    duration numeric(10,6) )")

artist_table_create = ("CREATE TABLE IF NOT EXISTS artists( \
    artist_id text PRIMARY KEY, name text, location text, \
    latitude numeric(10,6), longitude numeric(10,6) )")

time_table_create = ("CREATE TABLE IF NOT EXISTS time( \
    start_time timestamp, hour smallint, day smallint, week smallint, \
    month smallint, year smallint, weekday smallint \
 )")
...
create_table_queries = [user_table_create, song_table_create, 
    artist_table_create, songplay_table_create, time_table_create]